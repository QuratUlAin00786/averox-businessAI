...
async function createTables(client: postgres.Sql) {
  try {
    ...
    const maintenanceRequestsExists = await client`
      SELECT EXISTS (
    ...
    if (!maintenanceRequestsExists[0].exists) {
      ...
      console.log('maintenance_requests table already exists, skipping creation');
    }

    // Check if quality_checks table exists
    const qualityChecksExists = await client`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_schema = 'public' 
    ...
    if (!qualityChecksExists[0].exists) {
      ...
      console.log('quality_checks table already exists, skipping creation');
    }

    // Check if quality_check_parameters table exists
    const qualityCheckParametersExists = await client`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND table_name = 'quality_check_parameters'
      );
    `;
    
    if (!qualityCheckParametersExists[0].exists) {
      ...
      console.log('quality_check_parameters table already exists, skipping creation');
    }
