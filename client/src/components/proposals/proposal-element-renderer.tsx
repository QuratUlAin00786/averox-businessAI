...
export const getDefaultContent = (type: string) => {
  try {
    ...
    console.error("Error in getDefaultContent:", error, type);
    return {};
  }
};
...
export function ElementPreview({ element, isSelected }: ElementPreviewProps) {
  if (!element || !element.elementType) {
    return (
      <div className="border border-red-200 bg-red-50 p-3 my-2 rounded">
        <h3 className="font-medium text-red-500">Invalid element</h3>
      </div>
    );
  }
  ...
  try {
    ...
    try {
      ...
      console.error("Error merging content with defaults:", error, element);
    }
    ...
    console.error("Error rendering element preview:", error, element);
    return (
      <div className="border border-red-200 bg-red-50 p-3 my-2 rounded">
        <h3 className="font-medium text-red-500">Error rendering element</h3>
        <p className="text-xs text-red-400">
          Type: {element.elementType || 'unknown'}, ID: {element.id || 'unknown'}
        </p>
      </div>
    );
  }
...
export function ElementRenderer({ element }: { element: ProposalElement }) {
    ...
    return (
      <Card className="mb-4 border-red-200">
        <CardContent className="p-4">
          <div className="text-red-500">Invalid element</div>
        </CardContent>
      </Card>
    );
  }
  ...
  try {
    ...
    console.error("Error rendering full element:", error, element);
    return (
      <Card className="mb-4 border-red-200">
        <CardContent className="p-4">
          <div className="text-red-500">Error rendering element</div>
          <div className="text-sm text-red-400">Element ID: {element.id || 'unknown'}, Type: {element.elementType || 'unknown'}</div>
        </CardContent>
      </Card>
    );
  }
...
function renderElementContent(type: string, content: any = {}, fullView: boolean = false) {
  try {
    ...
    console.error("Error in renderElementContent:", error, { type, content });
    return (
